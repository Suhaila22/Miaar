...
const resolveApiUrl = () => `${ENV.llmApiUrl}/chat/completions`;

const assertApiKey = () => {
  if (!ENV.llmApiKey) {
    throw new Error(
      "LLM API key is not configured. Set OPENAI_API_KEY (or LLM_API_KEY for a custom OpenAI-compatible provider)."
    );
  }
};
...
export async function listLLMModels(): Promise<ModelsResponse> {
  ...
  const url = `${ENV.llmApiUrl}/models`;
